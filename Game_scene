...
class GameScene: SKScene {
 ...
 createLives()
 createSlices()
 sequence = [.oneNoBomb, .oneNoBomb, .twoWithOneBomb, .twoWithOneBomb, .three, .one,
.chain]
 for _ in 0 ... 1000 {
 ...
 enemy = SKSpriteNode(imageNamed: "blackcat")
 ...
 // 3. create a random X velocity (how far to move horizontally) that takes into account the
enemy's position
 ...
 // 5. give all enemies a circular physics body where the `collisionBitMask` is set to 0 so they
don't collide
 enemy.physicsBody = SKPhysicsBody(circleOfRadius: 64)
 enemy.physicsBody?.velocity = CGVector(dx: randomXVelocity * physicsVelocityMultiplier,
dy: randomYVelocity * physicsVelocityMultiplier)
